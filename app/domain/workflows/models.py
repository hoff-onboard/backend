...
class Step(BaseModel):
    ...
    def reject_bad_selectors(cls, v: str) -> str:
        if _PLAYWRIGHT_PSEUDO_RE.search(v):
            raise ValueError(f"Selector contains Playwright pseudo-selector: {v!r}")
        if _POSITIONAL_RE.search(v):
            raise ValueError(f"Selector contains positional pseudo-class: {v!r}")
        if _DYNAMIC_ID_RE.search(v):
            raise ValueError(f"Selector contains dynamic/random ID: {v!r}")
        if _REACT_ID_RE.search(v):
            raise ValueError(f"Selector contains React-style dynamic ID: {v!r}")
        if _HASH_CLASS_RE.search(v):
            raise ValueError(f"Selector contains hash/random class name: {v!r}")
        return v
